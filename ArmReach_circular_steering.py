...
from myosuite.train.utils.wrapper import wrap_curriculum_training, EpisodeWrapper
...
from myosuite.envs.myo.myouser.circular_steering import CircularSteering
...
def main(experiment_id, project_id='mjx-training', n_train_steps=100_000_000, n_eval_eps=10,
  ...
  model_path = 'simhive/uitb_sim/circular_steering.xml'
  ...
  wrapped_env = wrapper.BraxAutoResetWrapper(wrapped_env)
  if get_env_only:
    return wrapped_env
...
def evaluate(env, render_env, inference_fn, n_eps=10, rng=None, times=[], experiment_id='Steering'):
  ...
  for episode in range(n_eps):
    ...
    while not (done or state.info['truncation']):
      states.append(state)
      #print(state.info.keys())
      #print(state.info['touching_screen'])
