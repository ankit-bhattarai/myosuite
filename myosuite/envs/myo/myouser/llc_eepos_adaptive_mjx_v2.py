...
class LLCEEPosAdaptiveDirectCtrlEnvMJXV0(mjx_env.MjxEnv):
    ...
    def __init__(self, model_path=None, frame_skip=25, # aka physics_steps_per_control_step
            eval_mode=False,
            seed=123, **kwargs):
        config = config_dict.create(gpu_id=0, 
                                    ctrl_dt=0.01, 
                                    sim_dt=0.01,
                                    episode_length=1000,
                                    action_repeat=1,
                                    vision=True,)
        super().__init__(config, config_overrides=None)
        ...
        if 'vision' in kwargs:
            ...
            self.batch_renderer = BatchRenderer(m = self._mjx_model,
                                                ...
                                                enabled_geom_groups = np.asarray([0, 1, 2]), #TODO: check if group 3 is needed
    ...
    def step(self, state: mjx_env.State, action: jp.ndarray) -> mjx_env.State:
        ...
        data = self.add_target_pos_to_data(data, state.info['target_pos']) #TODO: check if this is necessary or not!
    ...
    def generate_target_pos(self, rng, target_area_dynamic_width_scale, target_pos=None):
        ...
        # body_pos = self._mjx_model.body_pos
        # body_pos = body_pos.at[1].set(target_pos)
        # self._mjx_model = self._mjx_model.replace(body_pos=body_pos)
        return target_pos
    ...
    def reset_with_curriculum(self, rng, info_before_reset, **kwargs):
        ...
        if self.reset_type == "zero":
            ...
            reset_qpos, reset_qvel, reset_act = self._reset_zero(rng_reset)
            data = mjx_env.init(self._mjx_model, qpos=reset_qpos, qvel=reset_qvel, act=reset_act)
...
class AdaptiveTargetWrapper(Wrapper):
  ...
  def reset(self, rng: jax.Array, **kwargs) -> mjx_env.State:
