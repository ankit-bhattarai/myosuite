...
class LLCEEPosAdaptiveDirectCtrlEnvMJXV0(mjx_env.MjxEnv):
    ...
    def ensure_dependent_joint_angles(self, data):
        ...
        for (virtual_joint_id, physical_joint_id, poly_coefs) in zip(
            ...
            new_qpos = new_qpos.at[virtual_joint_id].set(jp.polyval(poly_coefs, data.qpos[physical_joint_id]))
            data = data.replace(qpos=jp.select([physical_joint_id >= 0], [new_qpos], old_qpos))
        return data
