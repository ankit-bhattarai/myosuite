...
class LLCEEPosAdaptiveDirectCtrlEnvMJXV0(MyoUserBase):
    def _setup(
        self,
        # target_pos_range: dict,
        # target_radius_range: dict,
        # target_origin_rel: list = jp.zeros(
        #     3
        # ),  # [0.225, -0.1, 0.05],  #NOTE: target area offset should be directly added to target_pos_range
        # ref_site="humphant",
        # adaptive_task=True,
        ...
        self.target_pos_range = self._config.target_pos_range
        self.target_radius_range = self._config.target_radius_range
        self.target_origin_rel = self._config.target_origin_rel
        self.ref_site = self._config.ref_site
        ...
        self.adaptive_task = self._config.adaptive_params is not None
        if self.adaptive_task:
            adaptive_params = self._config.adaptive_params
            self.init_target_area_width_scale = adaptive_params.init_target_area_width_scale
            self.adaptive_increase_success_rate = adaptive_params.adaptive_increase_success_rate
            self.adaptive_decrease_success_rate = adaptive_params.adaptive_decrease_success_rate
            self.adaptive_change_step_size = adaptive_params.adaptive_change_step_size
            self.adaptive_change_min_trials = adaptive_params.adaptive_change_min_trials
            self.success_log_buffer_length = adaptive_params.success_log_buffer_length
            assert (
                self.adaptive_change_min_trials >= 1
            ), f"At least one trial is required to assess the success rate for adaptively adjusting the target area width. Set 'adaptive_change_min_trials' >= 1 (current value: {self.adaptive_change_min_trials})."
        else:
            self.init_target_area_width_scale = (
                1.0  # sample from full target area for non-adaptive tasks
            )
        ...
        self.reach_metric_coefficient = self._config.reach_metric_coefficient
        ...
        if sites:
            for site in sites:
                self.tip_sids.append(
                    mujoco.mj_name2id(
                        self._mj_model, mujoco.mjtObj.mjOBJ_SITE.value, site
                    )
                )
        ...
        self._max_steps_without_hit = (
            4.0 / self.dt
        )  # corresponds to 4 seconds; note that self.dt=self._mjx_model.opt.timestep*self._n_frames

    def _prepare_after_init(self, data):
        # Define target origin, relative to which target positions will be generated
        self.target_coordinates_origin = data.site_xpos[
            mujoco.mj_name2id(self._mj_model, mujoco.mjtObj.mjOBJ_SITE, self.ref_site)
        ].copy() + jp.array(
            self.target_origin_rel
        )  # jp.zeros(3,)
    ...
    def step(self, state: mjx_env.State, action: jp.ndarray) -> mjx_env.State:
        ...
        state.info["target_pos"] = jp.select(
            [(state.info["target_success"] | state.info["target_fail"])],
            [
                self.generate_target_pos(
                    rng1, state.info["target_area_dynamic_width_scale"]
                )
            ],
            state.info["target_pos"],
        )
        state.info["target_radius"] = jp.select(
            [(state.info["target_success"] | state.info["target_fail"])],
            [self.generate_target_size(rng2)],
            state.info["target_radius"],
        )
        ...
        data = mjx_env.step(self._mjx_model, state.data, new_ctrl, self.n_substeps)
        ...
        _, state.info["rng"] = jax.random.split(
            rng, 2
        )  # update rng after each step to ensure variability across steps

        state.metrics.update(
            target_area_dynamic_width_scale=obs_dict["task_completed"]
            * obs_dict["target_area_dynamic_width_scale"],
    ...
    def get_obs_dict(self, data, info):
        ...
        obs_dict["ee_pos"] = jp.vstack(
            [
                data.site_xpos[self.tip_sids[isite]].copy()
                for isite in range(len(self.tip_sids))
            ]
        )
        obs_dict["target_pos"] = info[
            "target_pos"
        ]  # jp.vstack([data.site_xpos[self.target_sids[isite]].copy() for isite in range(len(self.tip_sids))])

        # Distance to target (used for rewards later)
        obs_dict["reach_dist"] = jp.linalg.norm(
            jp.array(obs_dict["target_pos"]) - jp.array(obs_dict["ee_pos"]), axis=-1
        )

        # Target radius
        obs_dict["target_radius"] = info[
            "target_radius"
        ]  # jp.array([self._mj_model.site_size[self.target_sids[isite]][0] for isite in range(len(self.tip_sids))])
        # jax.debug.print(f"STEP-Obs: {obs_dict['target_radius']}")
        obs_dict["inside_target"] = jp.squeeze(
            obs_dict["reach_dist"] < obs_dict["target_radius"]
        )
        ...
        _steps_inside_target = jp.select(
            [obs_dict["inside_target"]], [info["steps_inside_target"] + 1], 0
        )
        ...
        obs_dict["steps_inside_target"] = jp.select(
            [obs_dict["target_success"]], [0], _steps_inside_target
        )
        obs_dict["steps_since_last_hit"] = jp.select(
            [obs_dict["target_success"] | obs_dict["target_fail"]],
            [0],
            info["steps_since_last_hit"],
        )
        obs_dict["trial_idx"] = info["trial_idx"] + jp.select(
            [obs_dict["target_success"] | obs_dict["target_fail"]], jp.ones(1)
        )
        ...
        if self.adaptive_task:
            ...
            #     return _data
            _trial_success_log_pointer_value = jp.select(
                [obs_dict["target_success"], obs_dict["target_fail"]], [1, 0], -1
            )
            idx = jp.arange(info["trial_success_log"].shape[-1])
            obs_dict["trial_success_log"] = jp.where(
                (idx == info["trial_success_log_pointer_index"])
                & (_trial_success_log_pointer_value != -1),
                _trial_success_log_pointer_value,
                info["trial_success_log"],
            ).copy()
            obs_dict["trial_success_log_pointer_index"] = (
                jp.select(
                    [obs_dict["target_success"] | obs_dict["target_fail"]],
                    [
                        (info["trial_success_log_pointer_index"] + 1)
                        % self.success_log_buffer_length
                    ],
                    info["trial_success_log_pointer_index"],
                )
                .astype(jp.int32)
                .copy()
            )
            ...
            success_rate = jp.where(
                (n_targets_adj != 0),
                n_hits_adj / jp.where(n_targets_adj != 0, n_targets_adj, 1),
                0,
            )
            # print(f"SUCCESS RATE: {self.success_rate*100}% ({self.n_hits_adj}/{self.n_targets_adj}) -- Last Adj. #{self.n_adjs}")
            obs_dict["target_area_dynamic_width_scale"] = jp.select(
                [
                    (obs_dict["target_success"] | obs_dict["target_fail"])
                    & (n_targets_adj >= self.adaptive_change_min_trials)
                    & (success_rate >= self.adaptive_increase_success_rate)
                    & (info["target_area_dynamic_width_scale"] < 1),
                    (obs_dict["target_success"] | obs_dict["target_fail"])
                    & (n_targets_adj >= self.adaptive_change_min_trials)
                    & (success_rate <= self.adaptive_decrease_success_rate)
                    & (info["target_area_dynamic_width_scale"] > 0),
                ],
                [
                    info["target_area_dynamic_width_scale"]
                    + self.adaptive_change_step_size,
                    info["target_area_dynamic_width_scale"]
                    - self.adaptive_change_step_size,
                ],
                info["target_area_dynamic_width_scale"],
            ...
            obs_dict["trial_success_log"] = jp.select(
                [
                    (obs_dict["target_success"] | obs_dict["target_fail"])
                    & (n_targets_adj >= self.adaptive_change_min_trials)
                    & (success_rate >= self.adaptive_increase_success_rate)
            ...
            obs_dict["trial_success_log_pointer_index"] = jp.select(
                [
                    (obs_dict["target_success"] | obs_dict["target_fail"])
                    & (n_targets_adj >= self.adaptive_change_min_trials)
                    & (success_rate >= self.adaptive_increase_success_rate)
            ...
            obs_dict["target_area_dynamic_width_scale"] = info[
                "target_area_dynamic_width_scale"
            ]
            obs_dict["success_rate"] = (
                -1.0
            )  # unknown ##TODO: measure success rate if adaptive is not enabled
    ...
    def update_info(self, info, obs_dict):
        ...
        if self.adaptive_task:
            info["trial_success_log_pointer_index"] = obs_dict[
                "trial_success_log_pointer_index"
            ]
            info["trial_success_log"] = obs_dict["trial_success_log"]
        info["target_area_dynamic_width_scale"] = obs_dict[
            "target_area_dynamic_width_scale"
        ]
    ...
    def get_reward_dict(self, obs_dict):
        ...
        act_mag = (
            jp.linalg.norm(obs_dict["act"], axis=-1) / self._na if self._na != 0 else 0
        )
        ...
        rwd_dict = collections.OrderedDict(
            (
                ...
                (
                    "reach",
                    1.0 * (jp.exp(-reach_dist_to_target_bound * rm) - 1.0) / rm,
                ),  # -1.*reach_dist)
                (
                    "bonus",
                    1.0 * (obs_dict["target_success"]),
                ),  # 1.*(reach_dist<2*near_th) + 1.*(reach_dist<near_th)),
                ("neural_effort", -1.0 * (last_ctrl**2)),
                ("act_reg", -1.0 * act_mag),
                # ('penalty', -1.*(np.any(reach_dist > far_th))),
                # Must keys
                ("sparse", -1.0 * (jp.linalg.norm(reach_dist, axis=-1) ** 2)),
                ("solved", 1.0 * (obs_dict["target_success"])),
                (
                    "done",
                    1.0 * (obs_dict["task_completed"]),
                ),  # np.any(reach_dist > far_th))),
            )
        )
        # print(rwd_dict.items())
        rwd_dict["dense"] = jp.sum(
            jp.array([wt * rwd_dict[key] for key, wt in self.rwd_keys_wt.items()]),
            axis=0,
        )
        return rwd_dict
    ...
    def generate_target_pos(
        self, rng, target_area_dynamic_width_scale, target_pos=None
    ):
        ...
        if target_pos is None:
            ...
            for (site, span), _rng in zip(self.target_pos_range.items(), rngs):
                if self.adaptive_task:
                    span = self.get_current_target_pos_range(
                        span, target_area_dynamic_width_scale
                    )
                # sid = mujoco.mj_name2id(self._mj_model, mujoco.mjtObj.mjOBJ_SITE.value, site + '_target')
                new_position = self.target_coordinates_origin + jax.random.uniform(
                    _rng,
                    shape=self.target_coordinates_origin.shape,
                    minval=span[0],
                    maxval=span[1],
                )
    ...
    def get_current_target_pos_range(self, span, target_area_dynamic_width_scale):
        return target_area_dynamic_width_scale * (
            span - jp.mean(span, axis=0)
        ) + jp.mean(span, axis=0)
    ...
    def reset(self, rng, **kwargs):
        ...
        if self.adaptive_task:
            ...
            trial_success_log = -1 * jp.ones(
                self.success_log_buffer_length, dtype=jp.int32
            )
        ...
        info = {
            ...
            "target_area_dynamic_width_scale": jp.array(
                self.init_target_area_width_scale, dtype=jp.float32
            ),  # TODO: do not reset to initial value at the beginning of each episode!!!
            "rng": rng,
        ...
        info["target_pos"] = self.generate_target_pos(
            rng,
            info["target_area_dynamic_width_scale"],
            target_pos=kwargs.get("target_pos", None),
        )
        info["target_radius"] = self.generate_target_size(
            rng, target_radius=kwargs.get("target_radius", None)
        )
        ...
        metrics = {
            "target_area_dynamic_width_scale": 0.0,  # info['target_area_dynamic_width_scale'],
            "success_rate": 0.0,  # obs_dict['success_rate'],
        }  #'bonus': zero}
    ...
    def reset_with_curriculum(self, rng, info_before_reset, **kwargs):
        ...
        last_ctrl = jp.zeros(
            self._nu
        )  # inserting last_ctrl into pipeline_init is not required, assuming that reset_with_curriculum is never called during instatiation of an environment (reset should be used instead)
        ...
        if self.reset_type == "zero":
            ...
            reset_qpos, reset_qvel, reset_act = self._reset_range_uniform(
                rng_reset, data
            )
        else:
            reset_qpos, reset_qvel, reset_act = None, None, None
        ...
        info = {
            ...
            "target_area_dynamic_width_scale": info_before_reset[
                "target_area_dynamic_width_scale"
            ].copy(),  # TODO: do not reset to initial value at the beginning of each episode!!!
            "rng": rng_reset,
            "target_success": jp.array(False),
            "target_fail": jp.array(False),
            "task_completed": jp.array(False),
        ...
        if self.adaptive_task:
            info["trial_success_log_pointer_index"] = info_before_reset[
                "trial_success_log_pointer_index"
            ].copy()
        ...
        info["target_radius"] = self.generate_target_size(
            rng_reset, target_radius=kwargs.get("target_radius", None)
        )
        data = self.add_target_pos_to_data(data, info["target_pos"])
