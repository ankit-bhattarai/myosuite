...
def heading_angle_NUMPY(parametrization_fct, theta, delta=1e-5, angle_change=False, rel_vec=np.array([1, 0])
):
    if (isinstance(theta, np.ndarray) and theta.ndim > 0) or isinstance(theta, list):
        return np.vectorize(functools.partial(heading_angle_NUMPY, parametrization_fct=parametrization_fct, delta=delta))(theta=np.array(theta))
...
def heading_rotation_matrix_NUMPY(parametrization_fct, theta, delta=1e-5):
    fwd_angle, bwd_angle = heading_angle_NUMPY(parametrization_fct, theta, delta=delta, angle_change=False)
...
def tunnel_from_nodes_NUMPY(nodes, tunnel_size=None, ord=np.inf, width_height_constraints=None):
    ...
    _bwd_offset_right = np.vstack(list(map(lambda x: x.dot(angle_to_rot_matrix_NUMPY(-np.pi/2).dot(np.array([1, 0]))), bwd_rot_matrix)))
    _fwd_offset_right = np.vstack(list(map(lambda x: x.dot(angle_to_rot_matrix_NUMPY(-np.pi/2).dot(np.array([1, 0]))), fwd_rot_matrix)))
